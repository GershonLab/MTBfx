...
sim1000 <- list()
for(i in 1:nrow(resp1000)){
  sim1000[[i]] <- dichEngine(iparFull = spellPars[,2:4], uFull=resp1000[i,], calib=spellPars$calib,
                        targetProb=.5, minNI=15, maxNI=50, maxSE=.387, nonML_se = 99,
                        stepVal = .75, minTheta=-6, maxTheta=6, exp.cont=exp_cont_mat, startTheta = 1,
                        maxCycle=100, critScore=5e-4, seedval=((i*3)^2), verbose=F)
}

# extract estimated final score and plot against generating score

estTheta <- sapply(sim1000, function(y) y$Theta)
plot(estTheta, genTheta, xlab="Estimated", ylab="Generating",main="Score Recovery\nStart Item at Theta=0")
abline(0,1, col='red')
